...
import { randomChar, randomNumbers } from "../utils/common";
export default function SignUp(props) {
  ...
  const sendUserData = async () => {
    await blueRex
      ...
      .then((e) => {
        setLoading(false);
        if (blueRex.status == 422 || e === "ErrorDuringProcess") {
          setMessage({
            message: "Error occured during sign up! try again later",
            variant: "error",
          });
          return 0;
        } else if (e === "EmailExist") {
          setMessage({
            message:
              "Email is registered choose another one or login with exist username!",
            variant: "alert",
          });
          return 0;
          ...
          <div className='form-view fade-in'>
            ...
            <form onSubmit={handleForm} autoComplete='off'>
              ...
              <button type='submit' disabled={loading}>
                {loading ? <div className='loading-spinner'></div> : "JOIN US"}
              </button>
              <p className='a83x08'>
                Already a member?
                <Link href='sign-in'>
